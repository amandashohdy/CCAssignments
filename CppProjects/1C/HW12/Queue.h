...
class Queue
{
	public :
		...
		Queue<Type>(int size = 100) 
		{
			if (size <= 0)
			{
				cout << "Cannot make array of less than 1 element." << endl ;
				cout << "Creating array of default size 100." << endl << endl ;
				maxSize = 100 ;
			}
			else
			{
				cout << "Creating array of size " << size << "." << endl << endl ;
				maxSize = size ;
			}
			count = 0 ;
			frontOfQueue = 0 ;
		...
		void enqueue(Type addElement) 
		{
			...
			else cout << "Queue is full. Element cannot be added." << endl << endl ;
		}
		 ...
		 *     Parameter:   none
		 *     Return   :   temp (Type) - element at the front of the queue
		 ***************************************************************************/
		Type dequeue() 
		{
			assert(!isEmpty()) ;

			Type temp = arr[frontOfQueue] ;
