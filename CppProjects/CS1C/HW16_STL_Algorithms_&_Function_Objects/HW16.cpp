...
InputIterator find_improved(InputIterator first,InputIterator last, const EqualityComparable& val) 
    // find the first element in [first,last) that equals value
{
    while (*first != val && first != last) { first++ ; } // temp, replace when defining function
...
{
    while (first != last && !pred(*first)) { first++ ; } // temp, replace when defining function
    return first; // included so that incomplete lab code will compile
}
...
class Range_low_to_high {
    public :
        ...
        {
            return !(v_high < x) && !(v_low > x) ;
        }
        Range_low_to_high(Val low, Val high)
        : v_high{high}, v_low{low} {} 
...
// Q#1
// For an input iterator object, pre-increment, postincrement, and dereference 
// operations.
// For an equality comparable object, the equality and inequality operations. 


// Q#5
// The copy_if algorithm requires the input iterator and the output iterator to
// have the pre and post increment as well as the dereference operators. The input
// iterator would likely be const and the output would not be. The predicate would 
// need an overloaded function to take in the value types and return a boolean 
// value. Lastly, the value_type needs to have the equality and comparison operators. 
